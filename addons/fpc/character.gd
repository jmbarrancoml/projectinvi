...
@export_group("Lookable Detector")
@export var lookableDetector : RayCast3D
@export var maxDistanceToPick : float = 3.0
var lastLookedNode : Node
...
func _process(delta):
	...
	scanner.updateScannerRotation(HEAD.rotation.y)
...
func handle_ld():
	# Comprobar si el RayCast está colisionando
	if lookableDetector.is_colliding():
		var collision_point = lookableDetector.get_collision_point()
		var collider = lookableDetector.get_collider().get_parent()
		# print("Collision detected at: ", collision_point, " with: ", collider)

		if "lookable" in collider and collider.lookable == true:
			# print_debug("Entered in collision with lookable")
			lastLookedNode = collider
			displayText('Presiona "E" para  coger')
		else:  # TODO: Refactorizar
			hideText() 
	else:
		 # print("No collision detected.")
		if lastLookedNode != null:
			lastLookedNode = null
			hideText()

func displayText(text: String):
	var text_displayer = $UserInterface/TextDisplayer
	text_displayer.visible = true
	text_displayer.text = text

func hideText():
	var text_displayer = $UserInterface/TextDisplayer
	text_displayer.visible = false
